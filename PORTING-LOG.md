...
**retired batch-adapter note:** the scout originally called for "wrap in batch adapter for clan scope" to fan out N*4 lookups (N=100-500 clan members). that in-client build direction was retired; durable roster-scale provider fanout now belongs to the killclog.com API, not the RuneLite plugin.
